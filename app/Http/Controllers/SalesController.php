...
class SalesController extends Controller
{
		...
		if ($user_auth->can('sales_add')){
            ...
            \DB::transaction(function () use ($request) {
                ...
                foreach ($data as $key => $value) {
                    ...
                    $get_product = Product::where('id', $value['product_id'])->first();
                    $get_product->sold = '1';
                    $get_product->save();

                    // product_warehouse::where('product_id', $value['product_id'])->update([
    ...
    public function sale_attatchment_download($sale_id, $item_id)
    {
        return response()->download(public_path('storage/sale/sale_'.$sale_id.'_item_'.$item_id.'.pdf'));
    }
}
