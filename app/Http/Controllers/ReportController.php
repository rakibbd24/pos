...
class ReportController extends Controller
{
   ...
   public function report_profit_filter(Request $request  , $start_date , $end_date , $warehouse)
   {
    ...
    public function report_profit_new()
    {
        $user_auth = auth()->user();
		if ($user_auth->can('report_profit')){

            return view('reports.report_profit_new');

        }
        return abort('403', __('You are not authorized'));
    }

    public function report_profit_new_post(Request $request)
    {
        $user_auth = auth()->user();
		if ($user_auth->can('report_profit')){
            ...
            $end = explode('/',$dates[2]);
            $startDate = date('Y-m-d H:i:s',strtotime($start[2].'/'.$start[0].'/'.$start[1]));
            $endDate = date('Y-m-d',strtotime($end[2].'/'.$end[0].'/'.$end[1]))." 23:59:59";

            $data = true;
            $deposits = Deposit::whereBetween('date',[$startDate,$endDate])
                                ->select('deposit_category_id', 'date', DB::raw('SUM(amount) as total_deposit_amount'))
                                ->groupBy(['deposit_category_id'])
                                ->get();
            $expenses = Expense::whereBetween('date',[$startDate,$endDate])
                                ->select('expense_category_id', 'date', DB::raw('SUM(amount) as total_expense_amount'))
                                ->groupBy(['expense_category_id'])
                                ->get();
            return view('reports.report_profit_new', compact('deposits', 'expenses', 'data'));
        }
        return abort('403', __('You are not authorized'));
    }
